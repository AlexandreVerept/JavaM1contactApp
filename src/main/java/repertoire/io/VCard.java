...
public class VCard {
	...
	public  Person toPerson() {
		...
		if(stringDecomposition.length>=2) {
			//on parse la date de naissance en localdate, l'opération peut être risqué d'où une gestion d'exception
			try {
			newPerson.setBirthDate(LocalDate.parse(stringDecomposition[1]));
			}
			catch(DateTimeParseException e) {
				 System.err.println("Unable to parse the birthdate!");
				 newPerson.setBirthDate(null);//dans le cas où c'est un echec on préferera mettre la date de naissance à null
			}
		}
