...
public class PersonDaoTestCase {
	...
	public void shouldSearchPersons() throws SQLException, DaoException {
		...
		List<Person> personList = personDao.searchPersons("Gheysens", nullCaracter, nullCaracter, nullCaracter, nullCaracter, nullCaracter);
		assertThat(personList).hasSize(2).doesNotContainNull();
		
		personList = personDao.searchPersons(nullCaracter, "Daniel", nullCaracter, nullCaracter, nullCaracter, nullCaracter);
		assertThat(personList).hasSize(2).doesNotContainNull();
		
		personList = personDao.searchPersons(nullCaracter, nullCaracter, "Alex", nullCaracter, nullCaracter, nullCaracter);
		assertThat(personList).hasSize(2).doesNotContainNull();
		
		personList = personDao.searchPersons(nullCaracter, nullCaracter, nullCaracter,"0", nullCaracter, nullCaracter);
		assertThat(personList).hasSize(2).doesNotContainNull();
		
		personList = personDao.searchPersons(nullCaracter, nullCaracter, nullCaracter,nullCaracter,"123 rue bidon", nullCaracter);
		assertThat(personList).hasSize(3).doesNotContainNull();
		
		personList = personDao.searchPersons(nullCaracter, nullCaracter, nullCaracter,nullCaracter, nullCaracter, "alex@patate.gouv");
		assertThat(personList).hasSize(2).doesNotContainNull();
		
		// then two parameters that do not follow each other
		personList = personDao.searchPersons("Gheysens", nullCaracter, nullCaracter,nullCaracter, "123 rue bidon", nullCaracter);
		assertThat(personList).hasSize(2).doesNotContainNull();
		
		// the all parameters
		personList = personDao.searchPersons("Verept", "Alexandre","Alex","0606060606", "1 rue des patates", "alex@patate.gouv");
		assertThat(personList).hasSize(1).doesNotContainNull();
		
		// test if there is no match
		personList = personDao.searchPersons("George", "Le Rouge Gorge","Redman","56537538", "3 quartier de la jungle", "mdr@lol.xd");
		assertThat(personList).hasSize(0).doesNotContainNull();
	}
	
	@Test(expected = DaoAllParametersAreNullException.class)
	public void shouldSendDaoAllParametersAreNullExceptionSearchPersons() throws SQLException, DaoAllParametersAreNullException {
		PersonDao personDao = new PersonDao();
	...
	public void shouldAddPerson() throws Exception {
		PersonDao personDao = new PersonDao();
		Person personToCreate = new Person(Integer.valueOf(10),"Ginette","Six Tonnes","Gigi","911","graveyard","gigi@lol.com",LocalDate.of(2000, 12, 4));
